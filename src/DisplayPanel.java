...
public class DisplayPanel<T> extends JPanel {
   ...
   protected void paintComponent(Graphics g) {
     ...
     if(avl.getRoot().value.toString().length() <23*150){
          ...
          g.drawString(avl.getRoot().value.toString().substring(start,avl.getRoot().value.toString().length()),xs,ys);
     }
     ...
     int XSCALE, YSCALE;  
     XSCALE=SCREEN_WIDTH/avl.totalnodes; //scale x by total nodes in tree
     YSCALE=(SCREEN_HEIGHT-ys)/(avl.maxheight+1); //scale y by tree height
